  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ margin: '0 0 8px 0' }}>CSP - 打折买币</h2>
        <p style={{ margin: 0, color: '#666', fontSize: 14 }}>
          策略说明：卖出看跌期权（Put），目标以折扣价格接货或赚取权利金
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <input type="number" value={availableCash} onChange={e => setAvailableCash(e.target.value)} style={{ padding: 8, fontSize: 14 }} />
        </label>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead style={{ background: '#f8f9fa', position: 'sticky', top: 0 }}>
                <tr>
                  ...
                  <th style={{ padding: '10px 8px', textAlign: 'right', borderBottom: '2px solid #dee2e6' }}>盈亏平衡</th>
                  <th style={{ padding: '10px 8px', textAlign: 'right', borderBottom: '2px solid #dee2e6' }}>折扣%</th>
              ...
              <tbody>
                {result.candidates.map((c, idx) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '8px', borderBottom: '1px solid #eee', textAlign: 'right' }}>${formatNumber(c.breakeven, 2)}</td>
                    <td style={{ padding: '8px', borderBottom: '1px solid #eee', textAlign: 'right' }}>{(c.discount_pct * 100).toFixed(2)}%</td>
